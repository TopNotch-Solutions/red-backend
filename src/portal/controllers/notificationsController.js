...
console.log(newUserNotifications);
            ...
            if (allTokens.length > 0) {
                const payload = {
                    ...
                    data: { navigationId: 'Notifications', notificationId: notification.id.toString(),notificationType: "MadeForYou" },
                    tokens: allTokens
                };
...
exports.uploadNotificationImageHandler = async (req, res) => {
    if (!req.file) {
        return res.status(400).json({
            status: 'FAILED',
    ...
    return res.status(200).json({
        status: 'SUCCESS',
        message: 'Image uploaded. Pass the image path in sendNotification or sendNotificationToMultipleUsers socket events.',
...
exports.sendNotificationToMultipleUsers = async (req, res) => {
    let { userIds, title, message, scheduledAt } = req.body;
    let image = req.file ? req.file.path : null;

    if (userIds.length === 0) {
        return res.status(400).json({ status: "FAILED", message: "Please provide a valid user" });
    }
    ...
    try {
        ...
        if (validUsers.length === 0) {
            return res.status(404).json({ status: "FAILED", message: "No AppUser users found" });
        }
        ...
        const sendNotifications = async () => {
			...
			notificationType: "MadeForYou"
                    ...
                    console.log(`User ${userId} has no FCM token. Notification saved but no push sent.`);
                }
            }
        };
...
exports.sentNotifications = async (req, res) => {
    try {
        const notifications = await NotificationsModel.findAll({
           where: {
                // Forces binary (case-sensitive) comparison on the database level
                notificationType: sequelize.where(
                    sequelize.fn('BINARY', sequelize.col('notificationType')), 
                    'notification'
                )
            }
        });
