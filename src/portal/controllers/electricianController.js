...
exports.createElectrician = async (req, res) => {
    let { firstName, lastName, cellphoneNumber, email, streetName, suburb, town, userType } = req.body
...
exports.createRole = async (req, res) => {
    ...
    try{
        const existingRole = await RoleModel.findOne({ where: { role } });
...
exports.createRoleCategory = async (req, res) => {
    let { roleId,category } = req.body;

     if (!roleId || isEmpty(roleId)) {
        return res.status(400).json({
            status: "FAILED",
            message: "Role ID cannot be blank"
        });
    }
    if (!category || isEmpty(category)) {
        return res.status(400).json({
            status: "FAILED",
            message: "Category be blank"
        });
    }
    try{
        const existingRoleCategory = await RoleCategory.findOne({ where: { category } });
        ...
        await RoleCategory.create({ roleId, category });
...
exports.getElectricianById = async (req, res) => {
  ...
  try {
    const electrician = await usersModel.findByPk(userId, {
      attributes: {
        exclude: ['password', 'customerType', 'meterNumber', 'userType']
      }
    });
...
exports.unassignIssue = async (req, res) => {
	...
	if(issue.status === "Completed"){
        return res.status(400).json({
        status: "FAILED",
        message: "Issue has been completed. Re-assignment is not possible.",
      });
    }
