...
const balanceDetails = process.env.ENVIRONMENT === 'UAT' 
    ? process.env.BCX_BALANCE_DETAILS 
    : process.env.BCX_BALANCE_DETAILS_PROD;

const balance = process.env.ENVIRONMENT === 'UAT' 
    ? process.env.BCX_ACCOUNT_BALANCE
    : process.env.BCX_ACCOUNT_BALANCE_PROD;

const history = process.env.ENVIRONMENT === 'UAT' 
    ? process.env.BCX_PAYMENT_HISTORY
    : process.env.BCX_PAYMENT_HISTORY_PROD;

const idNumber = process.env.ENVIRONMENT === 'UAT' 
    ? process.env.BCX_ACCOUNT_PER_ID_NUMBER
    : process.env.BCX_ACCOUNT_PER_ID_NUMBER_PROD;

const accountDetails = process.env.ENVIRONMENT === 'UAT' 
    ? process.env.BCX_ACCOUNT_DETAILS
    : process.env.BCX_ACCOUNT_DETAILS_PROD;

const meterPerAccount = process.env.ENVIRONMENT === 'UAT' 
    ? process.env.BCX_METERS_PER_ACCOUNT
    : process.env.BCX_METERS_PER_ACCOUNT_PROD;

const consumption = process.env.ENVIRONMENT === 'UAT' 
    ? process.env.BCX_CONSUMPTION
    : process.env.BCX_CONSUMPTION_PROD;
...
exports.accountBalanceDetails = async (req, res) => {
    ...
    try {
        ...
        const fetchBalance = async () => {
            return await fetch(`${balanceDetails}`, {
                method: 'POST',
                headers: {
...
exports.accountDetails = async (req, res) => {
    ...
    try {
        ...
        const fetchAccountDetails = async () => {
            return await fetch(`${accountDetails}`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
...
exports.meters = async (req, res) => {
    ...
    try {
        ...
        const fetchMeters = async () => {
            return await fetch(`${meterPerAccount}`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
...
exports.consumption = async (req, res) => {
    ...
    try {
        ...
        const fetchConsumption = async () => {
            return await fetch(`${consumption}`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
