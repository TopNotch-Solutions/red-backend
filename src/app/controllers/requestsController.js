...
const { sendVendNotificationEmail } = require("../services/vendService");
const {
  sendVendNotificationEmailFailure,
} = require("../services/vendServiceFailure");

const partnerCode = process.env.ENVIRONMENT === 'UAT' 
    ? process.env.PARTNER_CODE_UAT
    : process.env.PARTNER_CODE_PROD;

const key = process.env.ENVIRONMENT === 'UAT' 
    ? process.env.SECURITY_KEY_UAT 
    : process.env.SECURITY_KEY_PROD;

const authCode = process.env.ENVIRONMENT === 'UAT' 
    ? process.env.LOCAL_AUTH_CODE_UAT 
    : process.env.LOCAL_AUTH_CODE_PROD;

const authName = process.env.ENVIRONMENT === 'UAT' 
    ? process.env.LOCAL_AUTH_NAME_UAT
    : process.env.LOCAL_AUTH_NAME_PROD;

const partnerName = process.env.ENVIRONMENT === 'UAT' 
    ? process.env.PARTNER_NAME_UAT 
    : process.env.PARTNER_NAME_PROD;

const syntelBaseURL = process.env.ENVIRONMENT === 'UAT' 
...
exports.vend = async (req, res) => {
  let { meterNumber, mcode, mobileNumber, amount, type, transactionType } =
    req.body;
  const userId = req.user.id;
  sendVendNotificationEmail({ meterNumber, amount, mobileNumber });
