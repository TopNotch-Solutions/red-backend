...
const MessagesModel = require("../../common/models/messagesModel")
...
exports.changeTaskStatus = async (req, res) => {
 ...
 if(status === "In Progress"){
       await IssueStatisticsModel.update(
      {
...
exports.getTaskStats = async (req, res) => {
  ...
  try {
    // 1. COMPLETED: Grouped by the month they were actually FINISHED
    const completedTasks = await IssueStatisticsModel.count({
      where: {
        electricianId: electricianId,
        dateCompleted: { [Op.ne]: null } // Only if it has a completion date
      },
      attributes: [
        [sequelize.fn("DATE_FORMAT", sequelize.col("dateCompleted"), "%Y-%m"), "month"]
      ],
      group: ["month"],
    });

    // 2. IN PROGRESS: Grouped by the month they were ACCEPTED
    const inProgressTasks = await IssueStatisticsModel.count({
      where: {
        electricianId: electricianId,
        dateAccepted: { [Op.ne]: null },
        dateCompleted: null // Still in progress means it hasn't been completed yet
      },
      attributes: [
        [sequelize.fn("DATE_FORMAT", sequelize.col("dateAccepted"), "%Y-%m"), "month"]
      ],
      group: ["month"],
    });

    // 3. PENDING: (Still using IssuesModel because Pending tasks might not have a stats record yet)
    ...
    // 4. HOURS: Summing from IssuesModel
    const hoursData = await IssuesModel.findOne({
      where: { electricianId },
      attributes: [
        [sequelize.fn("COALESCE", sequelize.fn("SUM", sequelize.col("overtime")), 0), "totalOvertime"],
        [sequelize.fn("COALESCE", sequelize.fn("SUM", sequelize.col("specialHours")), 0), "totalSpecialHours"],
      ],
      raw: true,
    });
    ...
    res.status(200).json({
      ...
      stats: {
        ...
        overtime: hoursData?.totalOvertime || 0,
        specialHours: hoursData?.totalSpecialHours || 0,
      },
    });
  } catch (error) {
